...
- [ ] **Download:** `https://gitlab.freedesktop.org/glvnd/libglvnd`
- [ ] **Action:** Build and install to `rootfs` to provide the primary `libGL.so` entry points.
...
- [ ] **GTK+ 3:** Clean and rebuild. It will now detect OpenGL support and enable hardware acceleration for UI elements (Will happen on next full build).

---

## Phase 7: Verification
Once the OS is booted, run these tests:
- [ ] **Check DRI Device:** `ls -l /dev/dri/card0` (should exist if kernel modules `virtio-gpu` or `i915` are loaded).
- [ ] **Check GLX Info:** Run `glxinfo | grep "OpenGL renderer"`.
- [ ] **Demo:** Run `glxgears`.
